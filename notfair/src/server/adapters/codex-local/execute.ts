...
export async function* executeCodexLocal(
  ...
  // --dangerously-bypass-approvals-and-sandbox is what makes NotFair
  // agents useful through Codex. Without it, codex auto-cancels every
  // MCP tool call (so set_shared_context / log_goal_action etc. fail)
  // and sandboxes loopback connections to our local MCP servers.
  // NotFair's trust model: the user installed this app, picked
  ...
  // its OWN bearer:
  //   - notfair-goals: shared per-machine secret
  //   - notfair-browser: same shared per-machine secret (both are
  //     internal NotFair servers on loopback)
  ...
  const mcpEnv: Record<string, string> = {
    [bearerEnvVarForServer("notfair-goals")]: mcpSecret,
